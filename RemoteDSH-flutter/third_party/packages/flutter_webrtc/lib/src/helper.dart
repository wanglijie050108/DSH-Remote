...
import 'native/ohos/audio_configuration.dart';
...
class Helper {
  ...
  /// Request capture permissions (camera/microphone) for Ohos.
  /// Ohos only. Returns `false` on non-ohos platforms (not "permission denied").
  static Future<bool> requestCapturePermission({
    bool camera = true,
    bool microphone = true,
  }) => OhosNativeAudioManagement.requestCapturePermission(
    camera: camera,
    microphone: microphone,
  );

  /// Enable or disable hardware speaker mute on Ohos.
  /// Ohos only. No-op on non-ohos platforms.
  static Future<void> setSpeakerMute(bool mute) =>
      OhosNativeAudioManagement.setSpeakerMute(mute);

  /// Enable or disable hardware noise suppressor on Ohos.
  /// Ohos only. Returns `false` on non-ohos platforms or if ADM is not initialized.
  static Future<bool> setNoiseSuppressorEnabled(bool enabled) =>
      OhosNativeAudioManagement.setNoiseSuppressorEnabled(enabled);

  /// Check if built-in acoustic echo canceller is supported on Ohos.
  /// Ohos only. Returns `false` on non-ohos platforms.
  static Future<bool> isBuiltInAcousticEchoCancelerSupported() =>
      OhosNativeAudioManagement.isBuiltInAcousticEchoCancelerSupported();

  /// Check if built-in noise suppressor is supported on Ohos.
  /// Ohos only. Returns `false` on non-ohos platforms.
  static Future<bool> isBuiltInNoiseSuppressorSupported() =>
      OhosNativeAudioManagement.isBuiltInNoiseSuppressorSupported();
}
